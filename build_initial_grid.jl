...
function get_initial_density(n, rho_c, r, dr)
    ...
    dM = 4π * rho .* r.^2 * dr  # Vectorized calculation of dM
...
function create_initial_grid(n, zones, R, rho_c)

    # Grid
    r = range(0, R, length=zones)
    dr = r[2] - r[1]

    rho, mass, dM = get_initial_density(n, rho_c, r, dr)
